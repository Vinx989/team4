...
class Utente:
    ...
    grado_amicizia = { '1' : 'amico standard', '2' : 'amico_stretto', '3' : 'parente'}
    ...
    def following(self, lista_utenti_oggetto=lista_utenti_oggetto):
        # il nome dell'amico è richiesto nella funzione indice_username
        index = self.indice_username(lista_utenti_oggetto)
        print('inserisci il grado di amicizia:')
        print(self.grado_amicizia)
        grado = input('')
        if index != False and grado in ['1','2','3']:
            self.amici_utente[lista_utenti_oggetto[index].username] = [grado, self.grado_amicizia[grado]]   
            print(self.amici_utente)
            print()
        else:
            print('Questo utente non esiste oppure il grado di amicizia non è valido.')
            print()
    ...
    def un_following(self):
        ...
        if self.amici_utente == {} :
            ...
            if exAmico not in self.lista_amici.keys():
                print('utente non presente nella lista di amici che segui')
            else:
                self.amici_utente.pop(exAmico, None)
                print(self.amici_utente)
